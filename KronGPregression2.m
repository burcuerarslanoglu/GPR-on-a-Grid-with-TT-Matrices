function [Mu, Sigma, timing, K_cross] = KronGPregression2(X, Xtrain, y, Xtest, hyp)
    ...
    K_cross = ones(M, N); % Initialize K_cross

    for d = 1:D
        diff = Xtest(:, d) - Xtrain(:, d)';
        K_cross = K_cross .* (sf_2 * exp(- (diff.^2) / (2 * l_2)));
    end
    timing.K_cross = toc;
